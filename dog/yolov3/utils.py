...
def draw_bbox(image, bboxes, CLASSES=TRAIN_CLASSES, show_label=False, show_confidence = False, Text_colors=(255,255,0), tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        bbox_color = (0,0,255)
        bbox_thick = int(2 * (image_h + image_w) / 1000)
        if bbox_thick < 1: bbox_thick = 1
        fontScale = 2 * bbox_thick
...
def detect_image(Yolo, b64, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.3, iou_threshold=0.45):
    original_image = cv2.imdecode(np.frombuffer(base64.b64decode(b64), dtype='uint8'), 1)
    ...
    _, encoded = cv2.imencode(".jpg",image_paded)
    base64_image = base64.b64encode(encoded.tobytes()).decode('utf-8')
        
    return base64_image, bboxes
...
# detect from webcam
def detect_video_realtime_mp(video_path, output_path, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', realtime=False):
    if realtime:
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    times, times_2 = [], []
    vid = cv2.VideoCapture(video_path)
